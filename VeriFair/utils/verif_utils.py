...
def load_student():
    ...
    # Ordinal features (already ordinal, keep as is)
    # Medu, Fedu, traveltime, studytime, failures, famrel, freetime, goout, Dalc, Walc, health: 0-5 scale
...
def in_const_uci(df, x, var_name, op, rhs):
    label_name = 'pass'
    dataframe = df.drop(labels = [label_name], axis=1, inplace=False)
    props = []
    for col in dataframe:
...
def in_const_domain_uci(df, x, x_, ranges, PA):
    label_name = 'pass'
    dataframe = df.drop(labels = [label_name], axis=1, inplace=False)
    #dataframe = df.drop(df.columns[len(df.columns)-1], axis=1, inplace=False)
    props = []
...
def in_const_diff_uci(df, x, x_, var_name, threshold):
    label_name = 'pass'
    dataframe = df.drop(labels = [label_name], axis=1, inplace=False)
    props = []
    for col in dataframe:
