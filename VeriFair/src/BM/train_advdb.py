# Test AIF360 Fair Classifiers - Bank Marketing Dataset (BM-1)
if __name__ == "__main__":
    ...
    class AdversarialDebiasingWithTransfer:
        ...
        def save_as_keras(self, output_path):
            ...
            n_features = weight_values[0].shape[0] if len(weight_values) > 0 else 16
    ...
    # Constraints for Bank Marketing dataset (adjust based on actual ranges)
    constraint = np.array([
        [0, 1],      # age (binary: 0=<25, 1=>=25)
        [0, 11],     # job (12 categories)
        [0, 3],      # marital (4 categories)
        [0, 7],      # education (8 categories)
        [0, 1],      # default (2 categories)
        [0, 2],      # housing (3 categories)
        [0, 2],      # loan (3 categories)
        [0, 1],      # contact (2 categories)
        [0, 9],      # month (10 categories)
        [0, 4],      # day_of_week (5 categories)
        [0, 5000],   # duration (range)
        [-4, 2],     # emp.var.rate (range)
        [1, 50],     # campaign (range)
        [0, 1000],   # pdays (range)
        [0, 10],     # previous (range)
        [0, 2]       # poutcome (3 categories)
    ])
    ...
    input_model_path = f'Fairify/models/bank/{ORIGINAL_MODEL_NAME}.h5'
    ...
    # ============================================================
    # SAVE REPAIRED MODEL AS BM-1-AdvDb.h5
    # ============================================================
    output_model_path = f"Fairify/models/bank/{ORIGINAL_MODEL_NAME}-AdvDb.h5"
    print(f"\n{'='*60}")
    ...
    if "AdversarialDebiasing" in results:
        ...
        evaluator_adv = FairnessEvaluator(
            wrapper_adv_random,
            constraint,
            protected_attribs=[0],  # age is now at index 0
