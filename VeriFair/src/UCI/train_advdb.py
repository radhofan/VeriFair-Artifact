# Test AIF360 Fair Classifiers - UCI Student Dataset
if __name__ == "__main__":
    ...
    class AdversarialDebiasingWithTransfer:
        ...
        def save_as_keras(self, output_path):
            ...
            n_features = weight_values[0].shape[0] if len(weight_values) > 0 else 30
    ...
    # === Load dataset ===
    print("Loading UCI Student dataset...")
    ...
    # Constraint for UCI Student dataset (30 features)
    constraint = np.array([
        [0, 1],   # school
        ...
        [0, 5],   # Walc
        [0, 5],   # health
        [0, 3]    # absences (binned into 5 bins: 0-4)
    ])
    ...
    input_model_path = f'Fairify/models/uci/{ORIGINAL_MODEL_NAME}.h5'
    ...
    # ============================================================
    # SAVE REPAIRED MODEL AS UCI-1-AdvDb.h5
    # ============================================================
    output_model_path = f"Fairify/models/uci/{ORIGINAL_MODEL_NAME}-AdvDb.h5"
    print(f"\n{'='*60}")
    ...
    if "AdversarialDebiasing" in results:
        ...
        evaluator_adv = FairnessEvaluator(
            wrapper_adv_random,
            constraint,
            protected_attribs=[1],  # sex is at index 1 in UCI Student dataset
